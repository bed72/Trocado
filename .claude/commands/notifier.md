...
- **Invalidação cross-feature após mutação** — na ramificação de sucesso do `_submit()` (ou equivalente), chamar `ref.invalidate(otherProvider)` diretamente para cada provider de leitura que precisa refetch. É o idioma canônico do Riverpod e funciona mesmo em providers `@Riverpod(keepAlive: true)`. **Exceção narrada**: esse é o único caso em que um notifier pode importar providers de outra feature — `ref.invalidate` é permitido, `ref.watch`/`ref.read.notifier` não.
