...
void main() {
  ...
  late MockGetBudgetSummary getBudgetSummary;
  ...
  group('BudgetBloc', () {
    group('BudgetLoaded', () {
      blocTest<BudgetBloc, BudgetState>(
        ...
        setUp: () {
          when(
            () => budgetRepository.findActive(any()),
          ).thenReturn(Right(budget));
          when(
            () => getBudgetSummary(any()),
          ).thenReturn(Right(summary));
        },
        build: buildBloc,
        act: (bloc) => bloc.add(const BudgetLoaded()),
        expect: () => [
          isA<BudgetState>()
              .having((s) => s.status, 'status', BudgetStatus.loading),
          isA<BudgetState>()
      ...
      blocTest<BudgetBloc, BudgetState>(
        ...
        expect: () => [
          isA<BudgetState>()
              .having((s) => s.status, 'status', BudgetStatus.loading),
          isA<BudgetState>()
              .having((s) => s.status, 'status', BudgetStatus.empty),
        ],
      );
      ...
      blocTest<BudgetBloc, BudgetState>(
        ...
        expect: () => [
          isA<BudgetState>()
              .having((s) => s.status, 'status', BudgetStatus.loading),
          isA<BudgetState>()
              .having((s) => s.status, 'status', BudgetStatus.error)
    ...
    group('BudgetCreated', () {
      blocTest<BudgetBloc, BudgetState>(
        ...
        build: buildBloc,
        act: (bloc) => bloc.add(BudgetCreated(
          amount: 1000.0,
    ...
    group('BudgetRecalculated', () {
      blocTest<BudgetBloc, BudgetState>(
        ...
        setUp: () {
          ...
          when(
            () => getBudgetSummary(any()),
          ).thenReturn(Right(updatedSummary));
        },
        build: buildBloc,
        seed: () => BudgetState(
          summary: summary,
