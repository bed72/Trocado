...
- Em mudanças não triviais, use as skills locais do OpenCode em `.opencode/skills/`: `trocado-sdd` para o ciclo OpenSpec, `trocado-batched-delivery` para planejar e executar lotes revisáveis, `trocado-architecture` para implementação e revisão, e `trocado-quality-gate` antes de declarar um lote ou mudança concluído ou arquivável.
- Mudanças OpenSpec não triviais devem organizar `tasks.md` em lotes verticais ou fundações coerentes, cada um integrável, verificável e limitado a 20 arquivos únicos alterados. Mudanças mecânicas de checkbox e `Estado do lote` no próprio `tasks.md` não entram nessa contagem; código, testes, migrations, configuração, manifests, lockfiles, documentação, coleções de API e alterações substantivas de specs entram.
- Execute lotes com `/apply-batch <change>` e aprove com `/apply-batch <change> approve <n>`. Aplique exatamente um lote por execução e respeite `PENDING → IMPLEMENTING → AWAITING_REVIEW → APPROVED`; somente aprovação humana explícita do lote permite o próximo, e aprovar nunca inicia o lote seguinte na mesma execução.
