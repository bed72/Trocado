...
Dependências de código: `Presentation → Application → Domain` e `Infrastructure → Application/Domain`. Domain não importa nenhuma camada externa. Application conhece Domain, seus próprios contratos e, quando precisa de transação, `Core\Application\Ports\TransactionPort`; não conhece implementações de Infrastructure ou Presentation. Os UseCases de Expense obtêm o ID autenticado por `Expense\Application\Ports\UserPort` e o passam explicitamente aos Repositories, sem importar Identity. O container liga os contratos às implementações nas bordas.
