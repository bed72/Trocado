...
- **WHEN** um User anterior a Authentication permanece no banco após a consolidação
...
**Reason**: `CreateUserUseCase` e `UserRepository::create` mantêm um segundo lifecycle que gera principal sem credencial conhecida e impede posterior SignUp com o mesmo e-mail.
**Migration**: Consumidores devem usar `POST /api/authentication/sign-up`; criação administrativa futura deverá ser definida como convite e ativação em mudança própria.
