...
`ListExpensesUseCase` consulta `ExpenseRepository::listByUser` sob `ScopePort`, que abre uma transação PostgreSQL e associa o proprietário à política RLS. A página contém `ExpenseEntity` e os cursores de navegação, e a Presentation monta o documento JSON:API e seus links. Hoje o cache geral é `file` no exemplo de ambiente (fallback `database` em `config/cache.php`), mas o rate limiter usa o store `redis` independentemente e os testes selecionam `array`. Redis já está configurado como serviço local. A listagem paginada está definida na mudança ativa `list-owned-expenses-with-cursor-pagination`; esta proposta se aplica à listagem resultante sem mudar seus contratos.
...
2. **Cache pertence a Expense, não ao Repository nem ao Domain.** Um contrato de capacidade `ExpenseListCachePort` em Expense Application permite ao `ListExpensesUseCase` buscar ou carregar uma página e aos UseCases de escrita invalidar a conta; o Adapter de Infrastructure implementa cache e serialização com Laravel. A consulta no miss permanece dentro de `ScopePort`, com filtro explícito por usuário e RLS. No hit, a identidade confiável recebida pelo caso de uso determina a chave; cache não concede acesso por cursor ou por chave e a borda HTTP continua autenticando. O Repository segue responsável por consulta e persistência, sem conhecer cache. Alternativas descartadas: facade na Application, invalidar no Controller, cachear JSON:API/links ou esconder o cache no Repository.
...
4. **Escrita e consistência.** `CreateExpenseUseCase` invalida a conta depois de `ScopePort::execute()` retornar com sucesso, quando a transação interna já foi confirmada; tentativas revertidas não invalidam. Edição e exclusão individual, quando forem criadas, adotarão a mesma regra para o proprietário afetado. Não executar efeito de cache no callback transacional com retry. Uma leitura concorrente pode observar dados anteriores por uma janela curta; a invalidação após commit e o TTL de 60 segundos delimitam a inconsistência aceita, sem prometer consistência forte ou snapshot de cursores. Outros caminhos de escrita que contornem os UseCases (manutenção, SQL direto) só convergem pela expiração, a menos que invalidem explicitamente. A exclusão da conta em Identity não ganha dependência cross-context para invalidar Expense: a autenticação deixa de conceder acesso e as entradas expiram.
...
- [Dados de outra conta ou cursor antigo reaproveitados] → Chaves/tag por ID confiável, autenticação obrigatória e RLS nas consultas de miss; validar hits, misses e isolamento entre contas.
